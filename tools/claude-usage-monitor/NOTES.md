...
| 2026-03-24 | Fixed settings save bug — lambda callbacks (\_on\_data\_cb, \_on\_colour\_change\_cb) stored in settings dict were breaking json.dump. Fixed by filtering keys starting with "\_" in save\_settings(). Fixed widget visibility not persisting across sessions (was a symptom of the same save bug). Added SUPPORTERS tab to options panel — live Patreon supporter list fetched from GitHub JSON, themed scrollable list using ttk.Scrollbar with clam theme for correct colour theming, SUPPORT ON PATREON button at bottom. Fixed Launch on Startup — startup registry entry now always uses pythonw.exe (not python.exe) to prevent a blank console window appearing at boot and taking down the monitor when closed. Added Patreon support section + clickable badge to README. |
